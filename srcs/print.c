...
void print_packet_info(t_trame *trame, struct pcap_pkthdr packet_header) {
  ...
  for (unsigned i = 0, idx = 5; i < sizeof(t_mac_header); i++) {
  ...
  for (unsigned i = sizeof(t_mac_header), idx = 5; i < sizeof(struct iphdr) + sizeof(t_mac_header); i++) {
  ...
  for (unsigned i = 34; i < packet_header.len; i++) {
