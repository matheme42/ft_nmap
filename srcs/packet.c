...
static void create_tcp_packet(E_SCAN scan, struct sockaddr *src,
                              struct sockaddr *dst, t_packet *pkt) {
  t_scan flag;
  uint16_t dest_port;
  uint16_t src_port;
  uint32_t dest_addr;
  ...
  fill_TCP_Header(&pkt->tcphdr, convert_scan_name_to_flag(scan), src_port,
                  dest_port);
  pkt->tcphdr.check =
...
void send_packet(thread_data *data, int socket) {
  ...
  sendto(socket, &packet, sizeof(struct packet), 0,
